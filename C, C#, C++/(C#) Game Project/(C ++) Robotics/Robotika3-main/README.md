...
void drawHotspotOnMatrix(uint8_t hx, uint8_t hy) {
  if (!matrixEnabled) { mx.clear(); return; }
  mx.clear();
...
void setup() {
  ...
  for (auto path : redirects)
    server.on(path, HTTP_GET, [](AsyncWebServerRequest* r){ r->redirect("/"); });

  server.on("/toggle", HTTP_GET, [](AsyncWebServerRequest* r){
    matrixEnabled = !matrixEnabled;
    if (!matrixEnabled) mx.clear();
    savePrefs();
    r->send(200, "text/plain; charset=utf-8", matrixEnabled ? "matrix:on" : "matrix:off");
  });

  server.addHandler(&events);
  server.begin();

  // 10 Hz aparatinis timeris
...
void loop() {
  ...
  if (!frameFlag) { vTaskDelay(1); return; }
  frameFlag = false;

  float pix[64];
  amg.readPixels(pix);

  int hiIdx = 0; float hiVal = pix[0];
  for (int i=1;i<64;i++){ if(pix[i]>hiVal){hiVal=pix[i]; hiIdx=i;} }
  uint8_t hx = hiIdx % 8;
  uint8_t hy = hiIdx / 8;
  ...
  if (hiVal > recMaxC + 0.05f) {
    recMaxC = hiVal; recX = hx; recY = hy;
    savePrefs();
  }

  String json = "{\"temps\":[";
  for (int i=0;i<64;i++){ json += String(pix[i],1); if(i<63) json += ","; }
  json += "]}";
