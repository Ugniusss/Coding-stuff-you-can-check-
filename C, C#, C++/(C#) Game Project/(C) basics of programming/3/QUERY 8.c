...
int findCourse(char course[30], char courses[10][30], int load);
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(findCourse("Algebra", s.courses, s.load) && findCourse("Calculus", s.courses, s.load)){ // *** first filter, conditions on the student
