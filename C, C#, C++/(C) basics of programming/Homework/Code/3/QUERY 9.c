...
int main(int argc, char *argv[]) {
	...
	if (db){
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            for (int i = 0; i < s.load; ++i){ // check if studies philosophy
                if(strcmp(s.courses[i], "Philosophy") == 0){
                    skip = 0;
                    break;
                }
            }

            for (int i = 0; i < s.load; ++i){ // check if bad grade from philosophy
                if(strcmp(s.courses[i], "Philosophy") == 0){
                    if (s.grades[i] < 5) {
                        skip = 1;
                    }
            ...
            if(skip == 1) {
                continue;
            } // skips the code below if student do not have "Philosophy" and grade under 4
